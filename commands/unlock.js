        ...
        .then(channels => {
            channels.forEach(channel => {
                if(((channel.parent&&!exp.includes(channel.parent.id))||(!channel.parent&&!exp.includes(channel.id)))&&(allow.includes(channel.type))) {
                    if (!alwaysLock.includes(channel.id)) {
                        channel.permissionOverwrites.edit(everyone, {SEND_MESSAGES: null})
                        message.channel.send("`Đã mở khóa channel/category: " + channel.name+ "`")
                    }
                }
            })
        })
